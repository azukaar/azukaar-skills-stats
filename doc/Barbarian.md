# **BARBARIAN** *(Warrior → Raw Power & Consumption)*

## **Core Philosophy:**
**Resource Consumer** - Burns through food for explosive power  
**Burst Damage Specialist** - High-impact, devastating abilities  
**Primal Warrior** - Raw strength, intimidation, and survival instincts

---

## **Entry Skills** *(No Aspect Requirements)*

### **Powerful Strike** (1sp, max 3)
- **Effect:** Next attack's damage multiplied by +50% per level (30sec cooldown)
- **Max Benefit:** ×2 damage at level 3
- **Purpose:** Single-hit burst damage
- **Type:** Active

### **Iron Stomach** (1sp)
- **Effect:** Eat raw meat safely as if it were cooked, no hunger/poison effects
- **Purpose:** Foundation for Consumption branch
- **Type:** Passive

---

## **Consumption Branch**

### **Gluttony** (2sp, max 5)
- **Prerequisite:** Iron Stomach
- **Aspect Requirement:** Warrior 10
- **Effect:** Active — Gain a stack of Glutony (potion effect, infinite time). Glutony consumes your hunger (1 food per 5 sec at level I, 1 food per sec at level 5). Activate once for level I, and more for subsequent levels until max stack number (which depends on level). +10% damage per level
- **Max Benefit:** Gluttony V
- **Type:** Active
- **Purpose:** Core resource mechanic for the entire tree

### **Quick Eating** (2sp)
- **Prerequisite:** Gluttony
- **Aspect Requirement:** Warrior 20
- **Effect:** Instant eat, no animation delay
- **Type:** Passive

### **Gorge** (3sp)
- **Prerequisite:** Quick Eating
- **Aspect Requirement:** Warrior 30
- **Effect:** Active — instantly fill hunger bar from inventory food
- **Type:** Active

### **Auto-Feed** (5sp)
- **Prerequisite:** Gorge
- **Aspect Requirement:** Warrior 50
- **Effect:** Passive — automatically eat from inventory when hunger drops low
- **Type:** Passive
- **Purpose:** Quality of life, sustain Gluttony without manual eating

### **Efficient Digestion** (2sp, max 3)
- **Prerequisite:** Gluttony
- **Aspect Requirement:** Warrior 20
- **Effect:** Reduces gluttony passive hunger usage (10% per level)
- **Max Benefit:** 30% reduced hunger cost
- **Type:** Passive

### **Meat Lover** (2sp)
- **Prerequisite:** Efficient Digestion
- **Aspect Requirement:** Warrior 30
- **Effect:** Meat restores +50% more hunger
- **Type:** Passive

### **Carnivorous** (3sp)
- **Prerequisite:** Meat Lover
- **Aspect Requirement:** Warrior 40
- **Effect:** Meat consumed by Gorge or Auto-Feed grants Carnivorous  (30 sec potion effect) — Gluttony consume another 50% less food
- **Type:** Passive
- **Purpose:** Synergy with Hunter's meat production

### **Feeding Edge** (2sp)
- **Prerequisite:** Gluttony
- **Aspect Requirement:** Warrior 20
- **Effect:** Sharpness/Power enchantment levels multiplied by Gluttony level while Carnivorous
- **Type:** Passive
- **Purpose:** Direct damage scaling from Gluttony

### **Gluttonous Weapon** (2sp, max 3)
- **Prerequisite:** Feeding Edge
- **Aspect Requirement:** Warrior 30
- **Effect:** Restore hunger on kill while Carnivorous
- **Max Benefit:**: 2 hunger points restored per kill
- **Type:** Passive

### **Devouring Strike** (4sp)
- **Prerequisite:** Gluttonous Weapon
- **Aspect Requirement:** Warrior 50
- **Effect:** Consume ALL Gluttony stacks — deal +2 mult damage per stack consumed (when Gluttony is consummed, apply all the debufs as if it normally expired).
- **Max Damage:** x10 at Gluttony V
- **Type:** Active
- **Purpose:** Ultimate burst finisher

---

## **Axe Mastery Branch**

### **Axe Focus** (2sp)
- **Prerequisite:** Powerful Strike
- **Aspect Requirement:** Warrior 10
- **Effect:** +3 damage with axes
- **Type:** Passive

### **Axe Power** (1sp, max 3)
- **Prerequisite:** Axe Focus
- **Aspect Requirement:** Warrior 20
- **Effect:** +1 damage per level with axes, -1 damage with swords
- **Max Benefit:** +3 additional damage
- **Type:** Passive

### **Crushing Blow** (2sp, max 2)
- **Prerequisite:** Axe Power
- **Aspect Requirement:** Warrior 30
- **Effect:** +1 knockback and +1 damage per level with axes, -1 damage with swords
- **Max Benefit:** +2 knockback
- **Type:** Passive

### **Floor Slam** (4sp, 3 levels)
- **Prerequisite:** Crushing Blow
- **Aspect Requirement:** Warrior 40
- **Effect:** AoE ground pound attack — damages all enemies in radius
- **Type:** Active
- **Purpose:** Axe ultimate ability

---

## **Sword Mastery Branch**

### **Sword Focus** (2sp)
- **Prerequisite:** Powerful Strike
- **Aspect Requirement:** Warrior 10
- **Effect:** +3 damage with swords
- **Type:** Passive

### **Sword Power** (1sp, max 3)
- **Prerequisite:** Sword Focus
- **Aspect Requirement:** Warrior 20
- **Effect:** +1 damage per level with swords, -1 damage with axes
- **Max Benefit:** +3 additional damage
- **Type:** Passive

### **Parry** (2sp, max 2)
- **Prerequisite:** Sword Power
- **Aspect Requirement:** Warrior 30
- **Effect:** 10% Chance to block incoming attacks per level when holdin a sword and +1 damage with sword, -1 damage with axes
- **Max Benefit:** 20% block chance
- **Type:** Passive

### **Giant Sweep** (4sp, 3 levels)
- **Prerequisite:** Parry
- **Aspect Requirement:** Warrior 40
- **Effect:** Wide arc attack — hits all enemies in front of you
- **Type:** Active
- **Purpose:** Sword ultimate ability

---

## **Dominance Branch**

### **Intimidating Presence** (2sp)
- **Prerequisite:** Powerful Strike
- **Aspect Requirement:** Warrior 10
- **Effect:** Kills apply Fear effect to nearby mobs (30sec cooldown)
- **Type:** Passive

### **War Cry** (3sp)
- **Prerequisite:** Intimidating Presence
- **Aspect Requirement:** Warrior 20
- **Effect:** AoE Fear for 10 seconds
- **Type:** Active

### **Terrifying Roar** (2sp)
- **Prerequisite:** War Cry
- **Aspect Requirement:** Warrior 30
- **Effect:** War Cry also applies Weakness. Requires Gluttony II.
- **Type:** Passive

### **Demoralizing Shout** (2sp)
- **Prerequisite:** Terrifying Roar
- **Aspect Requirement:** Warrior 40
- **Effect:** War Cry also applies Slowness II for 30 seconds. Requires Gluttony III.
- **Type:** Passive

### **Berserker** (2sp, max 5)
- **Prerequisite:** Intimidating Presence
- **Aspect Requirement:** Warrior 30
- **Effect:** When Gluttony ends (food depleted), gain +200% damage per level for 10 seconds before becoming exhausted
- **Max Benefit:** +1000% damage
- **Type:** Passive (triggers automatically)
- **Purpose:** Emergency fallback when food runs out

### **Second Wind** (3sp)
- **Prerequisite:** Berserker
- **Aspect Requirement:** Warrior 40
- **Effect:** Full heal when Berserker triggers
- **Type:** Passive

### **Last Stand** (5sp)
- **Prerequisite:** Second Wind
- **Aspect Requirement:** Warrior 50
- **Effect:** At ≤1 heart while Exhausted, become invulnerable for 15 seconds (once per day)
- **Type:** Passive
- **Purpose:** Ultimate survival ability

---

## **Ultimate Capstone**

### **???** (5sp) 🔒
- **Prerequisites:** Devouring Strike + Auto-Feed + Last Stand
- **Aspect Requirement:** Warrior 75
- **Effect:** TBD
- **Type:** TBD
- **Purpose:** Ultimate Barbarian ability

---

## **Progression Summary**

| Warrior Aspect | Consumption | Axe | Sword | Dominance |
|----------------|-------------|-----|-------|-----------|
| **10** | Gluttony | Axe Focus | Sword Focus | Intimidating Presence |
| **20** | Quick Eating, Efficient Digestion, Feeding Edge | Axe Power | Sword Power | War Cry |
| **30** | Gorge, Meat Lover, Gluttonous Weapon | Crushing Blow | Parry | Terrifying Roar, Berserker |
| **40** | Carnivorous | Floor Slam | Giant Sweep | Demoralizing Shout, Second Wind |
| **50** | Auto-Feed, Devouring Strike | | | Last Stand |
| **75** | | | **??? (Capstone)** | |

---

## **Gluttony Mechanics**

### **Gluttony Stacking:**
1. Use Gluttony skill — consume hunger, gain Gluttony I
2. Use again — gain Gluttony II (up to max level = skill level)
3. Gluttony persists as long as you have food to sustain it
4. When food runs out — Gluttony ends, Berserker triggers (if skilled)

### **Gluttony Scaling:**
- **Gluttony I:** Sharpness/Power ×1
- **Gluttony II:** Sharpness/Power ×2, enables Terrifying Roar
- **Gluttony III:** Sharpness/Power ×3, enables Demoralizing Shout
- **Gluttony IV:** Sharpness/Power ×4
- **Gluttony V:** Sharpness/Power ×5, maximum Devouring Strike damage

### **Exhausted State:**
- Triggered by: Gluttony ending (Berserker) or Devouring Strike
- Duration: 5 minutes (Devouring Strike) or until fed (Berserker)
- Effect: Cannot stack Gluttony, weakened state
- Enables: Last Stand (at ≤1 heart while Exhausted)

---

## **Hunter Synergy**

The Barbarian + Farmer pairing feeds into Hunter paragon:

- **Farmer produces:** Abundant food, sustainable meat via Herd Communion
- **Barbarian consumes:** Meat Lover + Carnivorous reward meat specifically
- **Hunter bridges:** Pack mechanics + shared consumption bonuses

### **Key Synergy Skills:**
- **Carnivorous:** 2× Gluttony efficiency from meat (Hunter provides reliable meat)
- **Gluttonous Weapon:** Kills restore hunger (Hunter's pack increases kill rate)
- **Auto-Feed:** Sustains Gluttony automatically (Farmer's food stockpiles)

---

## **Total Investment**

### **Minimum for Capstone: ~41 SP**

*Consumption (18 SP):*
- Iron Stomach: 1 SP
- Gluttony (1 level): 2 SP
- Quick Eating: 2 SP
- Gorge: 3 SP
- Auto-Feed: 5 SP
- Feeding Edge: 2 SP
- Gluttonous Weapon (1 level): 2 SP
- Devouring Strike: 4 SP

*Dominance (15 SP):*
- Powerful Strike (1 level): 2 SP
- Intimidating Presence: 2 SP
- Berserker (1 level): 2 SP
- Second Wind: 3 SP
- Last Stand: 5 SP

*Capstone (5 SP):*
- ???: 5 SP

### **Full Tree Investment: ~85 SP**

*Entry (10 SP):*
- Powerful Strike (3): 3 SP
- Iron Stomach: 1 SP

*Full Consumption (24 SP):*
- Gluttony (5): 10 SP
- Quick Eating: 2 SP
- Gorge: 3 SP
- Auto-Feed: 5 SP
- Efficient Digestion (3): 3 SP
- Meat Lover (3): 3 SP
- Carnivorous: 3 SP
- Feeding Edge: 2 SP
- Gluttonous Weapon (3): 6 SP
- Devouring Strike: 4 SP

*Full Axe Mastery (11 SP):*
- Axe Focus: 2 SP
- Axe Power (3): 3 SP
- Crushing Blow (2): 4 SP
- Floor Slam: 4 SP

*Full Sword Mastery (11 SP):*
- Sword Focus: 2 SP
- Sword Power (3): 3 SP
- Parry (2): 4 SP
- Giant Sweep: 4 SP

*Full Dominance (19 SP):*
- Intimidating Presence: 2 SP
- War Cry: 3 SP
- Terrifying Roar: 2 SP
- Demoralizing Shout: 2 SP
- Berserker (5): 10 SP
- Second Wind: 3 SP
- Last Stand: 5 SP

*Capstone (5 SP):*
- ???: 5 SP